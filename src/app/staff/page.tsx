    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorPayload = { message: 'Resposta de erro não pôde ser lida' };
        }
        ...
        const serverMessage = typeof errorPayload === 'object' && errorPayload?.message 
          ...
          <div className="space-y-4 ">
            {ordersLoading ? (
              ...
              filteredOrders.map((order) => (
                <Card
                  key={order.id}
                  className="hover:shadow-lg transition-all hover:scale-[1.01] bg-yellow-200 cursor-pointer"
                    ...
                    <div className="flex flex-col space-y-3 sm:space-y-0 sm:flex-row sm:items-center sm:justify-between ">
                      ...
                      <div className="flex items-center justify-between sm:flex-col sm:items-end gap-2">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs sm:text-sm text-gray-600">{(order.items?.length ?? 0)} item(s)</p>
                        </div>
                      </div>
                    </div>
                  </CardHeader>
                            ...
                            {order.status !== OrderStatus.FINALIZADO && (
                              <>
                                ...
                                {order.status === OrderStatus.ENTREGUE && (
                                  <Button
                                    size="sm"
                                    onClick={() => updateOrderStatus(order.id, OrderStatus.FINALIZADO)}
                                    className="col-span-2 sm:col-span-1"
                                  >
                                    <CheckCircle className="h-4 w-4 mr-1" />
                                    <span className="hidden sm:inline">Marcar Finalizado</span>
