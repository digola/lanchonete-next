...
async function populateProducts() {
  try {
    console.log('🚀 Iniciando população de produtos no Supabase com UUIDs...');
    ...
    // Produtos para inserir (com IDs gerados manualmente)
    const products = [
      // Hambúrgueres
      {
        id: uuidv4(),
    ...
    console.log(`📦 Inserindo ${products.length} produtos com UUIDs gerados...`);
    ...
    for (const product of products) {
      try {
        ...
        // Inserir produto com UUID gerado
        ...
        if (error) {
          ...
          console.log(`✅ Produto "${product.name}" inserido com sucesso (ID: ${product.id})`);
          successCount++;
        }
      } catch (err) {
